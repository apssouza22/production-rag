...
async def _prepare_chunks_and_sources(
    ...
    return chunks, list(sources_set), arxiv_ids, query_embedding
...
async def ask_question(
    ...
    with rag_tracer.trace_request("api_user", request.query) as trace:
        try:
            # Check exact then semantic cache
            query_embedding = None
            if cache_client:
                try:
                    ...
                    if cache_result.response:
                        ...
                        logger.info("Returning %s cached response", hit_label)
            ...
            chunks, sources, _, query_embedding = await _prepare_chunks_and_sources(
                request, opensearch_client, embeddings_service, rag_tracer, trace, query_embedding
            )
            ...
            with rag_tracer.trace_prompt_construction(trace, chunks) as prompt_span:
                from src.domain.ollama.prompts import RAGPromptBuilder
            ...
            if cache_client:
                try:
                    await cache_client.store(request, response, query_embedding=query_embedding)
                except Exception as e:
            ...
            try:
                ...
                if cache_client:
                    try:
                        ...
                        if cache_result.response:
                            hit_label = "exact" if cache_result.hit_type == "exact" else "confidence-based"
                            logger.info("Returning %s cached response for streaming query", hit_label)
                ...
                if cache_client and full_response:
                    try:
                        ...
                        await cache_client.store(request, response_to_cache, query_embedding=query_embedding)
                    except Exception as e:
