...
from src.domain.health.schemas import HealthResponse, ServiceStatus
...
async def health_check(settings: SettingsDep, database: DatabaseDep, opensearch_client: OpenSearchDep) -> HealthResponse:
    ...
    try:
        ...
        services[settings.llm_provider] = ServiceStatus(status=llm_health["status"], message=llm_health["message"])
