...
class HybridIndexingService:
    ...
    2. Generating LLM context for each chunk (optional, Contextual Retrieval)
    3. Generating embeddings for each chunk
    4. Indexing chunks with embeddings into OpenSearch
    """
    ...
    def __init__(
        ...
        contextualizer: Optional[ChunkContextualizer] = None,
    ):
        ...
        :param contextualizer: Optional LLM-based chunk contextualizer
        """
        ...
        logger.info(
            "Hybrid indexing service initialized (contextualization=%s)",
    ...
    async def index_paper(self, paper_data: Dict) -> Dict[str, int]:
        ...
        try:
            ...
            # Step 2: Generate LLM context for each chunk (Contextual Retrieval)
            if self.contextualizer:
                full_text = paper_data.get("raw_text", paper_data.get("full_text", ""))
                chunks = await self.contextualizer.contextualize_chunks(
                    chunks=chunks,
                    document=full_text,
                    title=paper_data.get("title", ""),
                    abstract=paper_data.get("abstract", ""),
            ...
            # Step 3: Generate embeddings for contextualized chunk text
            chunk_texts = [chunk.get_contextualized_text() for chunk in chunks]
            ...
            # Step 4: Prepare chunks with embeddings for indexing
            chunks_with_embeddings = []

            for chunk, embedding in zip(chunks, embeddings):
                contextualized_text = chunk.get_contextualized_text()
                ...
                chunk_data = {
                    ...
                    "chunk_context": chunk.metadata.chunk_context,
