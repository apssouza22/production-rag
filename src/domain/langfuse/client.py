...
class LangfuseTracer:
    ...
    def trace_rag_request(
        ...
        Yields:
            LangfuseSpan for the request, or None if tracing is disabled.
        """
        if not self.client:
            yield None
            return
        ...
        try:
            with self.client.start_as_current_span(name="rag_request") as trace:
                trace.update(
                    input={"query": query},
    ...
    def create_span(
        ...
        """
        Create a child span within an active trace.

        Args:
            trace: Parent LangfuseSpan from trace_rag_request or start_as_current_span
        ...
        try:
            ...
            return self.client.start_span(name=name, input=input_data, metadata=metadata or {})
        except Exception as e:
